...
#   It also never fetches PR comments for a banner-compliance scan — that
#   scan was removed per operator review feedback on PR #27: it flagged the
#   operator's own human PR comments as "banner-less", which is noise, not a
#   real finding (a scan can't distinguish a skipped-banner bot comment from
#   a genuine human remark by the same PAT-backed author). Deterministic
#   banner enforcement lives entirely in cv-pr-comment.sh instead.
# ===========================================================================
...
assert_log_count "$GH_LOG" 'issues/.*comments' 0 "guard never fetches PR comments (banner-compliance scan removed)"
